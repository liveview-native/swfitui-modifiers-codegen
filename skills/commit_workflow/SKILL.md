...
```bash
# Completed assert() method (with test)
git add webidl/src/console/console.zig webidl/tests/console_test.zig
...
Includes comprehensive tests for true/false conditions and message formatting."

# Completed clear() method (with test)
git add webidl/src/console/console.zig webidl/tests/console_test.zig
git commit -m "feat: implement clear()

Empties group stack and message buffer per spec lines 75-80.

Tests verify group stack and buffer are properly emptied."

# Completed debug(), error(), info(), log(), warn() (with tests)
git add webidl/src/console/console.zig webidl/tests/console_test.zig
...
Tests verify proper delegation and log level handling."
```

### Example 1b: TDD Workflow (Test-First Approach)

If following strict TDD, you can optionally commit tests first:

```bash
# Write failing test first
git add webidl/tests/console_test.zig
git commit -m "test: add failing test for assert() method

Test verifies assert() behavior per WHATWG Console Standard."

# Implement feature to make test pass
git add webidl/src/console/console.zig webidl/tests/console_test.zig
git commit -m "feat: implement assert() with message prefixing

WHATWG Console Standard lines 52-72. Makes previously failing test pass."
```

**Note**: Both approaches are acceptable. Choose based on your workflow preference.

### Example 2: Abstract Operations (with Tests)

```bash
# Completed Logger (with test)
git add webidl/src/console/console.zig webidl/tests/console_test.zig
...
Tests verify format specifier detection and proper delegation."

# Completed Formatter (with test)
git add webidl/src/console/console.zig webidl/tests/console_test.zig
...
Comprehensive tests for all format specifiers and edge cases."

# Completed Printer (with test)
git add webidl/src/console/console.zig webidl/tests/console_test.zig
...
Tests verify buffering, output delegation, and memory cleanup."
```
...
1. ✅ **Tests written**: Implementation has corresponding test coverage
2. ✅ **Tests pass**: `zig build test`
3. ✅ **Code formatted**: `zig fmt webidl/src/console/`
4. ✅ **Build succeeds**: `zig build`
5. ✅ **No TODOs** introduced without tracking
6. ✅ **Documentation added** for public APIs
7. ✅ **Commit message** is descriptive

**Test-First Development (Optional but Recommended)**:
- Write failing test first
- Implement feature to make test pass
- Commit implementation + test together (or separately if following strict TDD)
